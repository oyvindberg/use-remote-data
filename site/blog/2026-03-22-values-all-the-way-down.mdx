...
For truly unrelated parts of the tree — a navbar and a settings panel that both need the current user, with no shared ancestor that makes sense — [`use-remote-data` has an opt-in `SharedStoreProvider`](/docs/shared-stores) with name-based deduplication, `gcTime` grace period, and automatic cleanup. It returns the same `RemoteDataStore<T>`, so `<Await>`, `.map()`, `RemoteDataStore.all()`, testing, and SSR all work identically.

The important thing is the *default*. When data is a prop by default and shared state is opt-in, most components end up independent, testable, and portable — without thinking about it. And when you do reach for shared state, you're making a conscious choice about which specific data to share, not opting your entire app into a global cache.
