...
| Dependency change | `user invalidating due to deps, from/to: [1] [2]` |
| Invalidation scheduled | `user: will invalidate in 30000` |
| Unmount cancels timer | `user: cancelled invalidation on unmount` |
...
`'invalidated-immediate'`, `'invalidated-initial'`, or `'invalidated-pending'`.
