...
It moves the store to the **`No`** state — which holds the error _and_ a `retry` callback that re-runs the exact same request.
...
When the user clicks retry, the store goes back to `Pending` and re-fires the request. If it succeeds this time, you get `Yes`. If it fails again, you're back to `No` with a fresh `retry`.
...
A failed request is just data — the `No` state — handled in the same place you render your success state.
...
- The combined store moves to `No`.
- The combined `retry` **only re-fetches the stores that failed**. The successful ones keep their data.
- Once the failing store succeeds, the combined store moves to `Yes` with the full tuple.
...
Pass your own `ErrorComponent` prop to `<Await>`:

```tsx
function MyErrorView({ errors, retry, storeName }: ErrorProps<never>) {
    return (
...
<Await store={store} ErrorComponent={MyErrorView}>
...
|---|---|---|
| `errors` | `Either<WeakError, E>[]` | The errors that caused the failure |
...
If a mutation fails, the store moves to `No`, and the default `<AwaitUpdate>` error view shows a retry button that re-runs with the **same parameters** that were originally passed.
