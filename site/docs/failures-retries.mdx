...
When the user clicks retry, the store goes back to `Pending` and re-fires the request. If it succeeds this time, you get `Yes`. If it fails again, you're back to `Failed` with a fresh `retry`.
