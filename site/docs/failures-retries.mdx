...
|---|---|---|
| `errors` | `Either<WeakError, E>[]` | The errors that caused the failure (see below) |
...
Each element is an `Either<WeakError, E>` — a tagged object that is one of two things:

- `{ tag: 'left', value: WeakError }` — an **unexpected error** (network failure, thrown exception, etc.). `WeakError` is just an alias for `Error | unknown`, because JavaScript lets you `throw` anything.
- `{ tag: 'right', value: E }` — a **typed domain error** you returned explicitly (only when using the [typed errors](typed-errors) feature).

If you're not using typed errors, every error will be `left` — so you can treat `error.value` as the thrown value. Most of the time you won't inspect this at all; the `retry` callback is enough.
