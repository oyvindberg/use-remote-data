...
This isn't just ergonomics. A forgotten null check is a bug that compiles. In domains where runtime errors are
expensive — fintech, healthcare, critical infrastructure — the difference between `T | undefined` and `T`
...
for exhaustive matching, or [neverthrow](https://github.com/supermacro/neverthrow) for typed errors,
you already think this way: make illegal states unrepresentable.
`use-remote-data` applies the same principle to async data.
...
| Stale data during refetch            | Yes (`isFetching` flag)                  | Yes (`isInvalidated` flag)                |
...
| Runtime dependencies                 | One (`@tanstack/query-core`)             | Zero                                      |

`react-query` is a great library with a huge ecosystem. If you need devtools, infinite queries, or automatic deduplication across components, use it.
