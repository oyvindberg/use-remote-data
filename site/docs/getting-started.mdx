...
In the example above, you can see that the store is passed to a `Await` component
...
Out of the box, `Await` handles loading and error states but is visually minimal,
including `JSON.stringify`ing error objects.

**Recommended usage** is therefore to copy/paste the `Await` component into your codebase,
