...
`use-remote-data` is fully compatible with the React Compiler (React 19+) and concurrent rendering
features like `startTransition` and Suspense boundaries.
...
- `useSharedRemoteData` uses `useSyncExternalStore` for tear-free concurrent reads
