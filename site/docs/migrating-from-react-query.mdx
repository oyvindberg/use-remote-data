...
| `useInfiniteQuery`                | `useRemoteDataMap` + local page list              | Each page is a key in the map. See [example below](#infinite-scroll).                                     |
...
Each page is an independent store entry — already-loaded pages stay rendered while the next page loads. If a page fails, its `<Await>` shows an error with retry, without affecting the other pages.
