...
function PostFeed() {
    ...
    const pages = useRemoteDataMap<number, PostPage>(
        (cursor, signal) => fetchPosts(cursor, signal)
    );

    const addPage = (nextCursor: number) =>
                ...
                <Await key={cursor} store={pages.get(cursor)}>
                    {(page) => (
                        <>
                            ...
                            {page.nextCursor && (
                                <button onClick={() => addPage(page.nextCursor)}>
                                    Load more
                                </button>
                            )}
                        </>
...
Each page is an independent store entry — already-loaded pages stay rendered while the next page loads. If a page fails, its `<Await>` shows an error with retry, without affecting the other pages. See the [Infinite Scroll](infinite-scroll) page for a runnable example.
