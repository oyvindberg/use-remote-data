...
The most common pattern: submit, invalidate dependent data, close the dialog.
...
<button
  onClick={() => saveStore.run({ name })}
  disabled={saveStore.current.type === 'pending'}
>
  Save
</button>
```
...
<WithRemoteUpdate store={previewStore}>
  {(result, run) => (
    <div>
...
Re-running while showing the previous result gives you the `InvalidatedPending` state automatically —
...
const saveStore = useRemoteUpdate(
  ...
  { invalidates: [userStore] },
);

// After save succeeds, userStore automatically re-fetches
```

This creates a clean unidirectional flow: **read → display → mutate → invalidate → re-read**.
