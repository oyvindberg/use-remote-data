...
`useRemoteUpdate` gives you the same typed state machine for writes that `useRemoteData` provides for reads.
...
or `Failed` (failure). The same `RemoteData` state machine you already know applies here.
...
The `refreshes` option does this for you. Pass the stores that should refresh:
