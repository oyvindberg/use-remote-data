...
The store starts in an `Initial` state. When `run()` is called, it transitions to `Pending`, then to `Yes` (success)
or `No` (failure). The entire `RemoteData` state machine you already know applies here too!
...
The `invalidates` option does this for you — just pass the stores that should refresh:

<Snippet snippet="mutation_invalidation" />

After success, each store in the array has its `invalidate()` called automatically.
Any `WithRemoteData` rendering those stores will re-fetch.
...
const store = useRemoteUpdate(
  (params) => saveItem(params),
  {
    invalidates: [itemsStore],
...
The callback runs once, after invalidation completes.
...
## Rendering with `WithRemoteUpdate`

For cases where you want to render the mutation result, use `WithRemoteUpdate`.
It works like `WithRemoteData` but **never auto-triggers** — fetching only starts when `run()` is called.
...
If a mutation fails, the `ErrorComponent` automatically shows a retry button — just like read stores.
