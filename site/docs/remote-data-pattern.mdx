...
function describe<T>(rd: RemoteData<T>): string {
  switch (rd.type) {
    ...
    case 'stale-immediate': return `Stale (just arrived): ${rd.stale.value}`;
    case 'stale-initial':   return `Stale (waiting): ${rd.stale.value}`;
    case 'stale-pending':   return `Refreshing: ${rd.stale.value}`;
