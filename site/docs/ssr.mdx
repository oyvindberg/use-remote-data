...
const store = useRemoteData(() => fetchProjects(), {
    initial: RemoteData.yes(serverData),
});
```

`<Await>` sees `Yes` on the very first render. No loading spinner. No hydration mismatch.
After hydration, the store works as usual — invalidation, re-fetching, mutations, retries all kick in.
...
import { RemoteData, useRemoteData, Await, InvalidationStrategy } from 'use-remote-data';
...
export function ProjectList({ serverData }: { serverData: Project[] }) {
    const store = useRemoteData(
        ...
        {
            initial: RemoteData.yes(serverData),
            invalidation: InvalidationStrategy.refetchAfterMillis(30_000),
        },
    );
...
2. **SSR** — `useState` initializes in `Yes`. `<Await>` renders the list. `useEffect` is skipped (never runs during SSR). The HTML already contains the project list.
3. **Hydration** — `useEffect(store.triggerUpdate)` fires. Store is `Yes`, so invalidation schedules a background re-fetch in 30 seconds.
4. **After 30s** — store invalidates, re-fetches in the background, `isRefreshing` goes `true`, new data arrives.
