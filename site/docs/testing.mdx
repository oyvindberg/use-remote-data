...
const loaded = RemoteDataStore.always(
  RemoteData.yes({ name: 'Alice', email: 'alice@example.com' })
);
...
test('dashboard renders when all data is loaded', () => {
  const userStore = RemoteDataStore.always(
    RemoteData.yes({ name: 'Alice' })
  );
  const postsStore = RemoteDataStore.always(
    RemoteData.yes([{ title: 'Hello world' }])
  );
...
test('shows dimmed text during background refresh', () => {
  const staleStore = RemoteDataStore.always(
    RemoteData.InvalidatedPending(
      RemoteData.Yes({ name: 'Alice' }, new Date())
    )
  );
...
export const Refreshing = () => (
  <UserCard store={RemoteDataStore.always(
    RemoteData.InvalidatedPending(
      RemoteData.Yes({ name: 'Alice (stale)', email: 'alice@example.com' }, new Date())
    )
  )} />
);
```
