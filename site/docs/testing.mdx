...
import { RemoteData, RemoteDataStore, Either } from 'use-remote-data';
...
test('renders an error with retry', () => {
  ...
  // Either.left() wraps an unexpected error (like a network failure).
  // The errors array is one element for a single store.
  const store = RemoteDataStore.always(
    RemoteData.Failed([Either.left(new Error('Network error'))], retry)
  );
  render(<UserCard store={store} />);
...
test('dashboard shows error when one request fails', () => {
  ...
  // Either.left() wraps an unexpected error — see "Failures and Retries" for details
  const postsStore = RemoteDataStore.always(
    RemoteData.Failed([Either.left(new Error('timeout'))], async () => {})
  );
...
test('shows dimmed text during background refresh', () => {
  ...
  expect(screen.getByText('Alice')).toBeInTheDocument();
  // Your component can use isInvalidated to style it differently
});
```
...
export const Failed = () => (
  <UserCard store={RemoteDataStore.always(
    RemoteData.Failed(
      [Either.left(new Error('Server returned 500'))],
      async () => {}
    )
  )} />
);
