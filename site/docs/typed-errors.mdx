...
Your fetch function returns a `Failure` instead of a plain value:

- `Failure.expected(value)` — success (the expected outcome)
- `Failure.unexpected(error)` — a domain error (not the outcome you wanted)

If the promise *rejects* (network failure, thrown exception), that's an unexpected error handled automatically.
If it *resolves* with `Failure.unexpected(...)`, that's a typed domain error you handle explicitly.
...
```tsx
import { z } from 'zod';
import { Failure, useRemoteDataEither, Await } from 'use-remote-data';
...
function fetchAndValidateUser(): Promise<Failure<z.ZodError, User>> {
...
const store = useRemoteDataEither(async () => {
    ...
    return Failure.unexpected(result); // PersonNotFound | PersonDeleted
});
```
