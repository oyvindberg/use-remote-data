...
you can choose to use `useRemoteDataEither` (or `useRemoteDataMapEither` for [dynamic data](dynamic-data))
...
You'll need to prove a custom error component to `Await`.
Typically, you can also create a specialized wrapper component around `Await` that uses your custom error-handling component.
